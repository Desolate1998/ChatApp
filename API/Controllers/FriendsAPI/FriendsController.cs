...
namespace API.Controllers.FriendsAPI
{
    public class FriendsController : Controller, IFriendsAPI
    {
        ...
        DeleteFriend(string DeleteingEmail, string Email)
        {
            throw new NotImplementedException();
        }

        [HttpGet("/Friends/GetAllRequests")]
        public async Task<List<FriendRequestsModel>>
        GetAllRequests([FromQuery] string Email)
        {
            List<FriendRequestsModel> Data =
                await friendServices.GetAllRequests(Email);
            return Data;
        }
        ...
        SendFriendRequest(
            [FromQuery] string SentToEmail,
            [FromQuery] string FromUser
        )
        {
            return await friendServices
                .SendFriendRequest(SentToEmail, FromUser);
        }

        [HttpGet("/Friends/GetChatMessages")]
        public async Task<List<Messages>>
        GetChatMessages([FromQuery] int StartIndex, [FromQuery] int ChatId)
        {
            return await friendServices.GetChatMessages(StartIndex, ChatId);
        }

        [HttpGet("/Friends/GetActiveChats")]
        public async Task<List<UserChatDisplay>>
        GetActiveChats([FromQuery] string Email)
        {
            return await friendServices.GetActiveChats(Email);
        }

        [HttpGet("/Friends/GetFriends")]
        public async Task<List<FriendModel>>
        GetFriends([FromQuery] string Email)
        {
        ...
        [HttpPost("/Friends/SendMessage")]
        public async Task SendMessage([FromBody] SendMessageModel Data)
        {
            await friendServices.SendMessage(Data);
        }

        [HttpPost("/Friends/StartActiveChat")]
        public async Task<List<Messages>> StartActiveChat([FromQuery] int chatId, [FromQuery] string Email)
        {
            return await friendServices.setChatActive(chatId, Email);
        }
    }
